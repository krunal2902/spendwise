<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-gray-800">Edit Group Expense</h2>
    </x-slot>

    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('groups.expenses.update', [$group, $expense]) }}" enctype="multipart/form-data" id="expenseForm">
            @csrf @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="amount" name="amount" type="number" step="0.01" class="mt-1 block w-full" :value="old('amount', $expense->amount)" required oninput="updateSplits()" />
                    ...
                    <div>
                        ...
                        <select id="paid_by" name="paid_by" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach($group->memberships as $membership)
                                <option value="{{ $membership->user_id }}" {{ old('paid_by', $expense->paid_by) == $membership->user_id ? 'selected' : '' }}>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4 flex flex-col">
                    ...
                    <div>
                        ...
                        <select id="split_type" name="split_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm mb-4" onchange="updateSplits()">
                            <option value="equal" {{ old('split_type', $expense->split_type) == 'equal' ? 'selected' : '' }}>Split Equally</option>
                            <option value="exact" {{ old('split_type', $expense->split_type) == 'exact' ? 'selected' : '' }}>Split by Exact Amounts</option>
                            <option value="percentage" {{ old('split_type', $expense->split_type) == 'percentage' ? 'selected' : '' }}>Split by Percentages</option>
                        </select>
                    </div>

                    <div id="splitContainer" class="flex-1 overflow-y-auto space-y-3">
                        @php
                            $splitMap = $expense->splits->keyBy('user_id');
                        @endphp
                        @foreach($group->memberships as $index => $membership)
                            @php
                                $hasSplit = $splitMap->has($membership->user_id);
                                $exactData = $hasSplit ? $splitMap[$membership->user_id]->amount : '';
                                $pctData = $hasSplit && $expense->split_type == 'percentage' ? $splitMap[$membership->user_id]->percentage : '';
                            @endphp
                            <div class="flex items-center justify-between p-2 rounded-lg bg-gray-50 border border-gray-200" 
                                 data-exact="{{ $exactData }}" data-pct="{{ $pctData }}">
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" id="user_{{ $membership->user_id }}" value="{{ $membership->user_id }}" {{ $hasSplit ? 'checked' : '' }} onchange="updateSplits()" class="split-checkbox rounded text-indigo-600">
                                    <label for="user_{{ $membership->user_id }}" class="text-sm font-medium text-gray-800">{{ $membership->user->name }}</label>
                                </div>
                                <div class="split-input-group hidden flex items-center gap-2"></div>
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <button type="submit" id="submitBtn" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-sm font-medium">Update Expense</button>
            </div>
    ...
    @push('scripts')
    <!-- Uses identical script block as create view -->
    <script>
